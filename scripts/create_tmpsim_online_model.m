...
create_tmpsim_dictionary(fullfile(modelFolder, 'tmpsim_online.sldd'));
...
add_line(modelName, 'cmd_valid_value/1', 'CommandAssembler/3', ...
    'autorouting', 'on');
add_line(modelName, 'hold_last_cmd_value/1', 'CommandAssembler/4', ...
    'autorouting', 'on');
add_line(modelName, 'control_mode_id_value/1', 'CommandAssembler/5', ...
    'autorouting', 'on');
end
...
for index = 1:numel(elementNames)
    ...
    add_line(subsystem, [constantName '/1'], ...
